...
public class Auto_Blue_1Cone_GrabCone_GrabCube extends SequentialCommandGroup {
  /** Creates a new Auto_Blue_1Cone_GrabCone_GrabCube. */
  public Auto_Blue_1Cone_GrabCone_GrabCube() {
     ...
     Commands.parallel(
      ...
      Commands.parallel(
        Commands.sequence(new RetrieveCone(),new PlaceConeUpperLevel()),
         Commands.sequence(new AdvAutoMove(4.5, -.1, 0, .1, .2, .05, 2, false), new AdvAutoMove(.5, -.1, 0, .1, .5, .05, 2, false),new AdvAutoMove(.5, -1.2, 0, .1, .3, .05, 2, false))),
    Commands.parallel(new setLiftPosition(Constants.liftMaxLevelConePOS),
         new setGantryPosition(Constants.gantryUpperLevelPOS), new setArmPosition(Constants.armExtendedPOS)),
     new setLiftPosition(Constants.liftMaxLevelConeDip),
     new setGripperPosition(Constants.openGripperPOS),
     Commands.parallel(
      Commands.sequence(Commands.parallel(new setArmPosition(Constants.armPickPOS), new setGantryPosition(Constants.gantryPickPOS)), 
            new setLiftPosition(Constants.liftBottomPOS)),
      Commands.sequence( new AdvAutoMove(.3, 0, 0, .3, .5, .1, 2, true), new AdvAutoMove(4, -.55, -170, .1, .5, .1, 2, false),  new AdvAutoMove(4.7, -.8, 180, .1, .2, .05, 2, false))),
      ...
      Commands.parallel(new RetrieveCube(), Commands.sequence(new AdvAutoMove(4.5, -.15, 180, .3, .5, .05, 2, false), new AdvAutoMove(1, -.15, 0, .2, .5, .1, 2, false)))
